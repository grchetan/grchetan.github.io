...
const CACHE_KEY = "site_leetcode_stats_cache_v4";
        ...
        if (res.ok) {
          ...
          if (data && data.totalSolved) {
            ...
            const result: LeetCodeStats = {
              username,
              total: Math.max(data.totalSolved ?? 0, fallback.total),
              easy: Math.max(data.easySolved ?? 0, fallback.easy),
              medium: Math.max(data.mediumSolved ?? 0, fallback.medium),
              hard: Math.max(data.hardSolved ?? 0, fallback.hard),
