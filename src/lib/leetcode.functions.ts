...
// Real submission calendar timestamps directly from LeetCode chetanprajapat07 profile
...
function buildCalendar(submissionCalendar: string): { date: string; count: number }[] {
  try {
    ...
    for (const [ts, count] of Object.entries(raw)) {
      ...
      // Map both UTC date string and Local date string so timezone shifts never miss a block
...
export async function getLeetCodeStats(username = "chetanprajapat07"): Promise<LeetCodeStats> {
  ...
  try {
    // 1. Try local dev proxy `/api/leetcode` or direct GraphQL
    const targetUrl = typeof window !== "undefined" && window.location.hostname === "localhost"
      ? "/api/leetcode"
      : "https://leetcode.com/graphql";

    const gqlRes = await fetch(targetUrl, {
      method: "POST",
    ...
    // 2. Try Alfa LeetCode Render Proxy API as backup
